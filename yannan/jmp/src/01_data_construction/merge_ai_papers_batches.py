"""
Merge AI Papers Batch Files

This script merges all batch parquet files from the ai_papers_batches directory
into a single comprehensive parquet file.

Usage:
    python merge_ai_papers_batches.py [--output OUTPUT_FILE]
"""
...
BATCH_DIR = PROJECT_ROOT / "data" / "processed" / "publication" / "ai_papers_batches"
OUTPUT_DIR = PROJECT_ROOT / "data" / "processed" / "publication"
DEFAULT_OUTPUT = OUTPUT_DIR / "ai_papers_complete.parquet"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(PROJECT_ROOT / "logs" / "merge_ai_papers_batches.log"),
...
def merge_batches(batch_dir: Path, output_file: Path, verify: bool = True):
    ...
    """
    logger.info("=" * 80)
    logger.info("MERGING AI PAPERS BATCH FILES")
    logger.info("=" * 80)
    ...
    try:
        ...
        # Read all batches lazily
        lazy_frames = []
        for i, batch_file in enumerate(batch_files, 1):
            logger.info(f"  Loading batch {i}/{len(batch_files)}: {Path(batch_file).name}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Merge AI papers batch files into a single parquet file",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Merge with default output file
  python merge_ai_papers_batches.py
  
  # Specify custom output file
  python merge_ai_papers_batches.py --output /path/to/output.parquet
  
  # Skip batch verification (faster but less safe)
  python merge_ai_papers_batches.py --no-verify
        """
    )
